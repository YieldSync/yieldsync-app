  ...
  return (
    <SectionShell>
      <div className="relative overflow-hidden">
        ...
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(70%_65%_at_50%_45%,transparent_0%,rgba(5,5,5,0.35)_100%)] light:bg-[radial-gradient(70%_65%_at_50%_45%,rgba(255,255,255,0.35)_0%,rgba(243,247,244,0.65)_100%)]" />
        ...
        <div className="relative z-10 flex min-h-[420px] flex-col items-center justify-center gap-5 px-6 py-24 text-center sm:min-h-[480px] sm:py-28">
          {!SIGNUPS_ENABLED ? (
            <p className="inline-flex items-center gap-2 border border-white/25 bg-white/10 px-3.5 py-1.5 text-[12px] font-semibold tracking-[0.04em] text-white uppercase backdrop-blur-sm light:border-primary/30 light:bg-primary/10 light:text-primary">
              <span className="size-1.5 rounded-full bg-primary shadow-[0_0_12px_var(--primary)]" />
              {LAUNCHING_SOON_TITLE}
            </p>
          ) : null}

          <h2 className="max-w-3xl text-balance text-[32px] font-bold leading-[1.1] tracking-[-0.03em] text-white light:text-foreground sm:text-[44px] md:text-[52px]">
            {content.title}
          </h2>

          <p className="max-w-xl text-pretty text-[15px] leading-relaxed text-white/80 light:text-muted-foreground sm:text-[17px]">
            {SIGNUPS_ENABLED ? content.description : LAUNCHING_SOON_BODY}
          </p>

          <Link
            href={SIGNUPS_ENABLED ? '#pricing' : publicAppHref('/login')}
